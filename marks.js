...
const rangeMark = {
    ...
    inclusive: true,
    parseDOM: [{tag: 'span[data-range]', getAttrs(dom) {
      return {
...
const rangeSelectionMark = {
    attrs: {
      ...
      inclusive: true,
      parseDOM: [{tag: 'span[data-range-selected]', getAttrs(dom) {
        return {active: dom.getAttribute('data-range-selected'), pmId: dom.getAttribute('data-pm-id')}
      }}],
      toDOM(node) {
          const { active, pmId } = node.attrs
          return ['span', { 'data-range-selected': active, 'data-pm-id': pmId }, 0]
      }
}
